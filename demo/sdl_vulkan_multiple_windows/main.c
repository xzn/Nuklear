...
bool recreate_swap_chain(struct vulkan_demo *demo, bool ctx) {
...
bool render(struct vulkan_demo *demo, struct nk_colorf *bg,
            VkSemaphore wait_semaphore, uint32_t image_index, bool ctx) {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        if (!recreate_swap_chain(demo, ctx)) {
...
struct window_thread_arg_t {
    struct vulkan_demo *vk_demo;
    struct nk_context *nk_ctx;
};

static bool program_running;
static bool nk_ctx_input;
static pthread_mutex_t nk_ctx_mutex = PTHREAD_MUTEX_INITIALIZER;

void *window_thread(void *arg) {
    struct window_thread_arg_t *win_arg = (struct window_thread_arg_t *)arg;
    struct vulkan_demo *demo = win_arg->vk_demo;
    struct nk_context *ctx = win_arg->nk_ctx;
    struct nk_colorf bg;
    struct nk_image img;
    uint32_t image_index;
    VkResult result;
    VkSemaphore nk_semaphore;
    bool need_nk_render = false;
    ...
    while (program_running) {
        int w, h;
        SDL_Vulkan_GetDrawableSize(demo->win, &w, &h);
        if (w != (int)demo->swap_chain_image_extent.width || h != (int)demo->swap_chain_image_extent.height)
            recreate_swap_chain(demo, ctx);
        ...
        pthread_mutex_lock(&nk_ctx_mutex);
        if (nk_ctx_input) {
            nk_input_end(ctx);
            nk_ctx_input = false;
        }
...
#endif
        /* ----------------------------------------- */
        pthread_mutex_unlock(&nk_ctx_mutex);
        need_nk_render = true;
nk_render:
        ...
        /* Draw */
        if (!ctx) {
            if (!render(demo, &bg, demo->image_available, image_index, ctx)) {
        ...
        need_nk_render = false;
        if (!render(demo, &bg, nk_semaphore, image_index, ctx)) {
            fprintf(stderr, "render failed\n");
            goto cleanup;
        }
    }
...
int main(void) {
    struct vulkan_demo vk_demo[2], *demo_0 = &vk_demo[0], *demo_1 = &vk_demo[1];
    VkResult result;
    int ret = -1;
    ...
    if (!create_instance(demo_0)) {
        return -1;
    }

    SDL_Window *win_0 = SDL_CreateWindow(
        "Demo 0", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WINDOW_WIDTH,
        WINDOW_HEIGHT,
        SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
    if (!win_0)
        goto cleanup_win_0;
    SDL_Window *win_1 = SDL_CreateWindow(
        "Demo 1", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WINDOW_WIDTH,
        WINDOW_HEIGHT,
        SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
    if (!win_1)
        goto cleanup_win_1;
    ...
    demo_0->win = win_0;
    if (!create_vulkan_demo(demo_0)) {
        fprintf(stderr, "failed to create vulkan demo!\n");
        goto cleanup_demo_0;
    }

    demo_1->instance = demo_0->instance;
    demo_1->win = win_1;
    if (!create_vulkan_demo(demo_1)) {
        fprintf(stderr, "failed to create vulkan demo!\n");
        goto cleanup_demo_1;
    }

    struct nk_context *nk_ctx = nk_sdl_init(win_0, demo_0->device, demo_0->physical_device,
        demo_0->indices.graphics, demo_0->overlay_image_views,
        demo_0->swap_chain_images_len, demo_0->swap_chain_image_format,
        0, MAX_VERTEX_BUFFER, MAX_ELEMENT_BUFFER);

    program_running = true;
    pthread_t thread_0 = 0;
    pthread_t thread_1 = 0;
    struct window_thread_arg_t thread_0_arg = { demo_0, nk_ctx };
    struct window_thread_arg_t thread_1_arg = { demo_1, NULL };
    if (pthread_create(&thread_0, NULL, window_thread, &thread_0_arg)) {
        program_running = false;
        goto cleanup_0;
    }
    if (pthread_create(&thread_1, NULL, window_thread, &thread_1_arg)) {
        program_running = false;
        goto cleanup_1;
    }
...
cleanup_win_0:
    ...
    if (demo_0->debug_messenger) {
        result = destroy_debug_utils_messenger_ext(demo_0->instance,
                                                   demo_0->debug_messenger, NULL);
        if (result != VK_SUCCESS) {
            fprintf(stderr, "Couldn't destroy debug messenger: %d\n", result);
        }
    }
    vkDestroyInstance(demo_0->instance, NULL);
